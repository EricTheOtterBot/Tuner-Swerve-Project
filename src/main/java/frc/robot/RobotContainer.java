...
public class RobotContainer {
    ...
    private final SwerveRequest.RobotCentric driveRobotCentric = new
    SwerveRequest.RobotCentric()
    .withDeadband(TunerConstants.MaxSpeed *
    0.02).withRotationalDeadband(TunerConstants.MaxAngularRate * 0.02)
    .withDriveRequestType(DriveRequestType.OpenLoopVoltage);
    ...
    public RobotContainer() {
        ...
        NamedCommands.registerCommand("Align To Tag 2", new RunCommand(
            () -> PPHolonomicDriveController.overrideXYFeedback(() -> {return 0.1;}, () -> {return 0.1;})));


//drivetrain.getPoseForAlignDSX(), 
//drivetrain.getPoseForAlignDSY(
//
        ...
        NamedCommands.registerCommand("Go Right", drivetrain.applyRequest(() -> driveRobotCentric.withVelocityY(-0.05 * TunerConstants.MaxSpeed).withVelocityX(0.0)));
    ...
    private void configureBindings() {
        ...
        joystick.povRight().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric.withVelocityY(-0.05 * TunerConstants.MaxSpeed).withVelocityX(0.0)));
        joystick.povLeft().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric.withVelocityY(0.05 * TunerConstants.MaxSpeed).withVelocityX(0.0)));
        joystick.povUp().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric.withVelocityX(0.05 * TunerConstants.MaxSpeed).withVelocityY(0.0)));
        joystick.povDown().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric.withVelocityX(-0.05 * TunerConstants.MaxSpeed).withVelocityY(0.0)));
        joystick.povUpLeft().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric.withVelocityX(0.05 * TunerConstants.MaxSpeed).withVelocityY(0.05 * TunerConstants.MaxSpeed)));
        joystick.povUpRight().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric.withVelocityX(0.05 * TunerConstants.MaxSpeed).withVelocityY(-0.05 * TunerConstants.MaxSpeed)));
        joystick.povDownLeft().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric.withVelocityX(-0.05 * TunerConstants.MaxSpeed).withVelocityY(0.05 * TunerConstants.MaxSpeed)));
        joystick.povDownRight().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric.withVelocityX(-0.05 * TunerConstants.MaxSpeed).withVelocityY(-0.05 * TunerConstants.MaxSpeed)));
    ...
    public Command pipeScoreCommand() {
        return new SequentialCommandGroup(
            new InstantCommand(() -> sSpaceGun.shootMotor(0.25), sSpaceGun),
            new WaitCommand(0.5),
    ...
    .withRotationalRate(
            drivetrain.getPoseForAlign().getRotation().getRadians()
                    * TunerConstants.MaxAngularRate)).until(drivetrain.isVisionTracked());
    }
